...
class DownloadPage(wx.Panel):
    ...
    SUCC_REPORT_MSG = "成功下载了{0}个URL，耗时{1}天{2}小时{3}分钟{4}秒"
    DL_COMPLETED_MSG = "下载完成"
    URL_REPORT_MSG = "总体进度：{0:.1f}% | 待下载({1}) 已暂停({2}) 正在下载({3}) 下载完成({4}) 出错({5})"
    CLOSING_MSG = "正在停止下载任务"
    CLOSED_MSG = "下载任务已停止"
    PROVIDE_URL_MSG = "您至少需要提供一个URL"
    DOWNLOAD_STARTED = "下载已经开始"
    CHOOSE_DIRECTORY = "选择目录"

    DOWNLOAD_ACTIVE = "有下载任务正在进行，请等待所有下载任务完成"
    UPDATE_ACTIVE = "更新已在进行中"

    UPDATING_MSG = "正在下载最新版youtube-dl，请稍候..."
    UPDATE_ERR_MSG = "youtube-dl下载失败[{0}]"
    UPDATE_SUCC_MSG = "成功下载youtube-dl"

    OPEN_DIR_ERR = "无法打开目录: '{dir}'。指定的路径不存在"
    SHUTDOWN_ERR = "关机时出错。请确保您输入了正确的密码"
    SHUTDOWN_MSG = "正在关机"

    VIDEO_LABEL = "标题"
    EXTENSION_LABEL = "文件扩展名"
    SIZE_LABEL = "大小"
    PERCENT_LABEL = "百分比"
    ETA_LABEL = "预计剩余时间"
    SPEED_LABEL = "速度"
    ...
    def __init__(self, parent, opt_manager, log_manager,status_bar):
        ...
        statuslist_menu_data = (
            ("Get URL", self._on_geturl),
            ("Get command", self._on_getcmd),
            ("Open destination", self._on_open_dest),
            ("Re-enter", self._on_reenter)
        )
    ...
    def _update_videoformat_combobox(self):
        ...
        if vformats:
            ...
            for item in vformats:
                self._videoformat_combobox.add_items(list(item),False)

        if aformats:
            self._videoformat_combobox.add_header("Audio")
            for item in aformats:
                self._videoformat_combobox.add_items(list(item),False)
        value = self.opt_manager.options["selected_format"]
        current_index = self._videoformat_combobox.FindString(FORMATS[value])
    ...
    def _update_videoformat(self, event):
        self.opt_manager.options["selected_format"] = selected_format = tdict(FORMATS)[self._videoformat_combobox.GetValue()]
        ...
        if selected_format in VIDEO_FORMATS:
            ...
            self.opt_manager.options["video_format"] = tdict(DEFAULT_FORMATS)["default"]
    ...
    def _on_viewlog(self, event):
        if self.log_manager is None:
            self._create_popup("Logging is disabled",
                               self.WARNING_LABEL,
                               wx.OK | wx.ICON_EXCLAMATION)
        else:
    ...
    def _create_bitmap_button(self, icon, size=(-1, -1), handler=None):
        button = wx.BitmapButton(self, bitmap=icon, size=size, style=wx.NO_BORDER)
    ...
    def _create_static_bitmap(self, icon, event_handler=None):
        static_bitmap = wx.StaticBitmap(self, bitmap=icon)
    ...
    def _create_textctrl(self, style=None, event_handler=None):
        if style is None:
            ...
            textctrl = wx.TextCtrl(self, style=style)
    ...
    #         self._create_popup("Updates are disabled for your system. Please use the system's package manager to update youtube-dl.",
    #                            self.INFO_LABEL,
    #                            wx.OK | wx.ICON_INFORMATION)
    #     else:
    ...
    def _update_from_item(self, row, download_item):
        ...
        for key in self.columns:
            ...
            if key == "status" and progress_stats["playlist_index"]:
                ...
                self.SetItem(row, column, status)   ## todo
            else:
                self.SetItem(row, column, progress_stats[key])    ## todo
